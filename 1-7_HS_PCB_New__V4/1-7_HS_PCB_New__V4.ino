...
#define sensor_UPDN   PD2
#define sensor_RIGH   PD3
#define sensor_LEFT   PD4
#define AC     PB6
...
void loop() {
  ...
  // if ( (PINA&(1<<sw_FOOT)) && (PINF&(1<<sw_SEMI)) ) { // 반자동 모드 Start
  if ( (btn == 'G') && (PINF&(1<<sw_SEMI)) ) { // 반자동 모드 Start
    
    if (PINF&(1<<sw_JIG)) time_count = 20000; // 진공 사용 여부

    if (!(PINF&(1<<sw_POS))) { 
      if (!(PIND&(1<<sensor_UPDN))) updn_H();
    }
  }
