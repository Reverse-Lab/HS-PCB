...
int optionTime;       // Jig 옵션 실행 시간 저장 변수
...
class Option
{
 ...
 void ON()
 {
  //if((jig == 1) && (optionOnTime >= 100)){
  if((jig == 1)){
    unsigned long optionCurrentMillis = millis();
    if((optionState == LOW) && (optionCurrentMillis - optionpreviousMillis >= optionOnTime))
    {
      optionState = HIGH;
